...
namespace nil {
    namespace crypto3 {
        namespace ffi {
            ...
            struct nil_crypto3_struct {
            public:
                nil_crypto3_struct(T *obj) : m_magic(MAGIC), m_obj(obj) {
                }

                virtual ~nil_crypto3_struct() {
                    m_magic = 0;
                    m_obj.reset();
                }
...
#define CRYPTO3_FFI_DECLARE_STRUCT(NAME, TYPE, MAGIC)                              \
    struct NAME final : public nil::crypto3::ffi::nil_crypto3_struct<TYPE, MAGIC> { \
        explicit NAME(TYPE *x) : nil_crypto3_struct(x) { }                          \
    }
            ...
            T &safe_get(nil_crypto3_struct<T, M> *p) {
                if (!p) {
            ...
            int apply_fn(nil_crypto3_struct<T, M> *o, const char *func_name, F func) {
                if (!o) {
...
#define CRYPTO3_FFI_DO(T, obj, param, block)                     \
    apply_fn(obj, BOOST_CURRENT_FUNCTION, [=](T &param) -> int { \
        ...
        return CRYPTO3_FFI_SUCCESS;                              \
            ...
            template<typename T, uint32_t M>
            int ffi_delete_object(nil_crypto3_struct<T, M> *obj, const char *func_name) {
                try {
                    if (obj == nullptr) {
                        return CRYPTO3_FFI_SUCCESS;
